        ...
        for root, dirnames, filenames in os.walk(tpldir):
            for filename in fnmatch.filter(filenames, '*.stpl'):
                if filename[0] <> '_':
                    ...
                    logging.info("loading template '{0}' from '{1}'".format(tplname, filename))
...
def create_app(uri_path='', rootdir=os.path.dirname(__file__), ticketRegistry = InMemoryTicketRegistry(), authenticator=DummyAuthenticator()):
    ...
    def get_request_params(*args):
        return tuple([request.params[k] if k in request.params else None for k in args])
    ...
    def credentials_requestor():
        ...
        response.set_header('Pragma', 'no-cache')
        response.set_header('Expires', to_rfc2822(datetime.now() - timedelta(days=365)))
        service, renew, gateway = get_request_params('service', 'renew', 'gateway')
        gateway = gateway in ['1', 'true']
        tgt = ticketRegistry.validate_ticket_granting_ticket(request.get_cookie('tgc'))
        if service:
            if renew:
                logging.info('Authentication renew explicitly requested. Proceeding with CAS login for service ' + service)
            elif tgt:
                logging.info("Valid ticket granting ticket detected.")
                st = ticketRegistry.new_service_ticket(service, tgt.username, tgt)
                service_url_with_ticket = make_url_with_extraparam(service, 'st', st)
                logging.info("User '"+tgt.username+"' authenticated based on ticket granting cookie. Redirecting to "+service_url_with_ticket)
                redirect(303, service_url_with_ticket) # response code 303 means "See Other" (see Appendix B in CAS Protocol spec)
                return
            elif gateway:
                logging.info("Redirecting unauthenticated gateway request to service " + service)
                redirect(303, service)
                return
        elif gateway:   
            logging.error("This is a gateway request but no service parameter was given!")
        else:
    ...
    def credentials_acceptor():
        username, password, service, lt = get_request_params('username', 'password', 'service', 'lt')
        ...
        if not ticketRegistry.validate_login_ticket(lt):
            ...
            valid = False
            try:
                valid = authenticator.check_credentials(username, password, service)
            except Exception as e:
                logging.exception('authenticator failure')
                
            if valid:
                logging.info("Credentials for username '{0}' successfully validated".format(username))
                ticketGrantingTicket = ticketRegistry.new_ticket_granting_ticket(username)
                logging.debug('tgt : ' + repr(ticketGrantingTicket.tgt))
                response.set_cookie('tgt', ticketGrantingTicket.tgt)
                logging.info("Ticket granting cookie '{0}' granted to user '{1}'".format(ticketGrantingTicket.tgt, username))
                if not service:
                    logging.info('No service parameter was provided')
                    return templateRenderer.render('auth_success', username = username)
                else:
                    st = ticketRegistry.new_service_ticket(service, username, tgt)
                    service_with_ticket = make_url_with_extraparam(service, 'st', st)
                    logging.info('Redirecting to ' + service_with_ticket)
                    redirect(303, service_with_ticket)
            else:
                logging.info("Invalid credentials for username '{0}'".format(username))
                response.status = 401
                return templateRenderer.render('loginform', lt = ticketRegistry.new_login_ticket(), error_message='Invalid credentials')
    
    @app.route(uri_path+'/logout')
    def logout():
        service, destination, continue_url, gateway = get_request_params('service', 'destination', 'url', 'gateway')
        ...
        if ticketGrantingTicket:
            ticketRegistry.delete_ticket_granting_ticket(ticketGrantingTicket.tgt)
            username = ticketGrantingTicket.username
            if username:
                ticketRegistry.delete_proxy_granting_tickets_for_username(username)
    ...
    def loginTicket_needs_post():
        abort(422, 'this uri only works for POST requests')
    ...
    def validate():
        service, ticket, renew = get_request_params('service', 'ticket', 'renew')
    ...
    def tkValidate(isproxy):
        service, ticket, pgt_url, renew = get_request_params('service', 'ticket', 'pgtUrl', 'renew')
        renew = renew in ['1', 'true']
        ...
        if isproxy:
            ...
            valid = not ticketRegistry.get_service_ticket(service, ticket) is None
        dic['valid'] = valid
        ...
        return templateRenderer.render('proxy_validate', params=dic)
    ...
    def proxy():
        ...
        pt = ticketRegistry.new_proxy_ticket(pgt, target_service) if valid else None
