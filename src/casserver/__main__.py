...
if __name__ == '__main__':
    ...
    p.add_argument('--assets', default=os.path.dirname(__file__)
                   + '/../../assets', help="location of static files")
    p.add_argument('--path', default='/cas', help='root path')
    p.add_argument('--debug', dest='debug', action='store_const',
    ...
    if path != '/':
        @app.route('/')
        def redir(): redirect(path)
    run(app=app, host=args.host, port=int(args.port),
        debug=args.debug, reloader=True)
