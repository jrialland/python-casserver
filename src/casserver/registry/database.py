...
class DbTicketRegistry(TicketRegistry):
    ...
    def new_service_ticket(self, service, username, tgt):
        ...
        db = self.db()
        db.execute(
            "insert into service_ticket(key, username, service, tgt) values(?,?,?,?)",
                   [st, username, service, tgt])
        db.commit()
        return ServiceTicket(st, username, service, tgt)

    def get_username_for_service_ticket(self, st):
        c = self.db().cursor()
        c.execute(
            "select tgt.username from service_ticket st join ticket_granting_ticket tgt on st.tgt=tgt.key where st.key=?", [st])
        r = c.fetchall()
    ...
    def get_service_ticket(self, service, st):
        c = self.db().cursor()
        c.execute(
            "select key, username, service, tgt from service_ticket where key=? and service=?", [st, service])
        r = c.fetchall()
    ...
    def new_ticket_granting_ticket(self, username):
        ...
        db = self.db()
        db.execute(
            "insert into ticket_granting_ticket(key, username) values(?,?)", [tgt, username])
        db.commit()
    ...
    def get_ticket_granting_ticket(self, tgt):
        c = self.db().cursor()
        c.execute(
            "select username from ticket_granting_ticket where key=?", [tgt])
        r = c.fetchall()
    ...
    def delete_proxy_granting_tickets_for_username(self, username):
        ...
        c = db.cursor()
        c.execute(
            "select key from ticket_granting_ticket where username=?", [username])
        sts = []
        ...
        for pgt in pgts:
            db.execute("delete from proxy_ticket where pgt=?", [pgt])
        db.commit()
    ...
    def new_proxy_granting_ticket(self, st, url):
        ...
        db = self.db()
        db.execute(
            "insert into proxy_granting_ticket(key, st,url) values(?,?,?)", [pgt, st, url])
        db.commit()
        return pgt
    ...
    def new_proxy_ticket(self, pgt, target):
        ...
        pt = 'pt-' + \
            (''.join(random.choice(string.ascii_lowercase + string.digits)
             for _ in range(32)))
        db.execute(
            "insert into proxy_ticket(key, pgt, target) values(?,?,?)", [pt, pgt, target])
        db.commit()
        return pt
